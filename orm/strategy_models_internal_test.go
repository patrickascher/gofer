...
func helperCreateDatabaseAndTable(asserts *assert.Assertions) {
	...
	asserts.NoError(err)
	_, err = b.Query().DB().Exec("CREATE TABLE `tests`.`role_roles` (`role_id` int(11) unsigned NOT NULL AUTO_INCREMENT, `child_id` int(11) unsigned NOT NULL, PRIMARY KEY (`role_id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;")
	asserts.NoError(err)

	createBuilderCache(asserts)
}

func createBuilderCache(asserts *assert.Assertions) {
	var err error
...
type AddressPoly struct {
	...
	Animal         *Animal `orm:"relation:belongsTo;fk:AnimalPolyID"`
